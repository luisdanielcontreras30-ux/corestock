...
import { analizarProveedorConIA, ResultadoAnalisisEntidad } from "../../../lib/iaAcciones";
...
export default function AnalisisModal({ proveedor, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const datos = await analizarProveedorConIA(proveedor.id, idioma);
        if (cancelado) return;
        setResultado(datos);
  ...
  function abrirWhatsApp() {
    window.open(enlaceWhatsApp(mensajeEditado, proveedor.telefono), "_blank");
  }
      ...
      <div className="factura-modal" style={{ maxWidth: 480 }} onClick={(e) => e.stopPropagation()}>
        <div className="factura-modal-toolbar">
          <h2 style={{ fontSize: 18, fontWeight: 700 }}>
            {t("analisis_ia.titulo_modal")} — {proveedor.nombre}
          </h2>
