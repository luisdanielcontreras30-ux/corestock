...
export default function VentasRapidasPage() {
  ...
  const [plazoDiasCredito, setPlazoDiasCredito] = useState(30);
  ...
  const faltaCliente = metodoPago === "credito" && nombreClienteCredito.trim() === "";
  ...
  async function confirmarCobro() {
    ...
    try {
      ...
      const { encoladoOffline, primerId } = await registrarVentaRapida(
        ...
        metodoPago,
        user.id,
        metodoPago === "credito" ? nombreClienteCredito.trim() : "",
        metodoPago === "credito" ? plazoDiasCredito : null
      );
      mostrarToast(
        encoladoOffline
                ...
                {metodoPago === "credito" && (
                  <div className="venta-rapida-efectivo">
                    <div>
                      ...
                      <input
                        type="text"
                        value={nombreClienteCredito}
                        onChange={(e) => setNombreClienteCredito(e.target.value)}
                      ...
                      <div className="venta-rapida-metodos">
                        {[15, 30, 60].map((dias) => (
                          <button
                            ...
                            type="button"
                            className={
                              plazoDiasCredito === dias ? "btn-primary" : "btn-secondary"
                            }
                            onClick={() => setPlazoDiasCredito(dias)}
                          >
                            {t(`ventas.plazo_${dias}`)}
                          </button>
                        ))}
                      </div>
                    </div>
