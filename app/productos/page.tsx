...
import { ImagePlus, Package, Plus, Sparkles, Eraser, Inbox, Star, MapPin, ChevronDown, Loader2 } from "lucide-react";
...
// Bloquea la tecla ANTES de que llegue a aparecer en el campo — así
// nunca se ve ni un instante una letra en precio/costo/stock, en vez
// de escribirla y quitarla después (que se alcanzaba a ver un parpadeo).
const TECLAS_PERMITIDAS_SIEMPRE = [
  "Backspace", "Delete", "Tab", "Escape", "Enter",
  "ArrowLeft", "ArrowRight", "ArrowUp", "ArrowDown", "Home", "End",
];

function bloquearTeclaNoEntero(e: React.KeyboardEvent<HTMLInputElement>) {
  if (e.ctrlKey || e.metaKey || TECLAS_PERMITIDAS_SIEMPRE.includes(e.key)) return;
  if (!/^[0-9]$/.test(e.key)) e.preventDefault();
}

function bloquearTeclaNoDecimal(e: React.KeyboardEvent<HTMLInputElement>) {
  if (e.ctrlKey || e.metaKey || TECLAS_PERMITIDAS_SIEMPRE.includes(e.key)) return;
  const valorActual = e.currentTarget.value;
  if (e.key === "." && !valorActual.includes(".")) return;
  if (!/^[0-9]$/.test(e.key)) e.preventDefault();
}
  ...
  // Se ejecuta DESPUÉS de que React ya pintó el formulario abierto
  // (a diferencia de llamar scrollTo dentro de editar(), que corría
  // antes de que el DOM tuviera el formulario expandido y por eso
  // no siempre llegaba a la posición correcta).
  useEffect(() => {
    if (editando !== null) {
...
setStockMinimo(borrador.stockMinimo);
  ...
  async function guardar() {
    ...
    try {
      ...
      mostrarToast(
        editando !== null
          ? t("productos.msg_actualizado")
          : t("productos.msg_creado"),
        "exito"
      );
  ...
  return (
    <>
      ...
      <div ref={formRef} className={`card productos-form${formularioAbierto ? "" : " productos-form-cerrado-movil"}`}>
  ...
  onKeyDown={bloquearTeclaNoDecimal}
...
/>
  ...
  <input
    ...
    onKeyDown={bloquearTeclaNoEntero}
  ...
  />
        ...
        <div className="productos-toolbar">
          <button onClick={guardar} className="btn-primary" disabled={guardando} style={{ display: "flex", alignItems: "center", gap: 8 }}>
            {guardando && <Loader2 size={15} style={{ animation: "girar 1.1s linear infinite" }} />}
            {guardando
