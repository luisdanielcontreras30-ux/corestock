...
// La lista "Selecciona una receta" ya no vive aquí — antes se
// repetía, idéntica, en la sección de Producción de arriba, y con
// todo en una sola página se veía como si la app mostrara dos veces
// lo mismo. Ahora ambas secciones comparten el mismo producto
// seleccionado (estado en page.tsx); esta solo arma/edita los
// ingredientes de la receta de lo que ya esté elegido arriba.
  ...
  return (
    <div className="card">
      {!productoSeleccionado ? (
        <p style={{ color: "var(--text-secondary)", fontSize: 13.5 }}>{t("fabricacion.selecciona_producto")}</p>
      ) : (
        <>
          <h2 style={{ marginBottom: 4 }}>{t("fabricacion.ingredientes_de_receta")}</h2>
          <p style={{ color: "var(--text-secondary)", fontSize: 13, marginBottom: 16 }}>{productoSeleccionado.nombre}</p>
